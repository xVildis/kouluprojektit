...
if($mode == "poista")
{
    $sql = "DELETE FROM vier_juttu WHERE jid = ?"; 
...
if($mode == "muokkaa") {
    if(!empty($_POST["otsikko"]) && !empty($_POST["kpl"]))     {
        ...
        $sql="UPDATE vier_juttu set otsikko=:otsikko,kpl=:kpl,poistamispvm=:poistamispvm,lisayspvm=:lisayspvm,kid=:kid WHERE jid=:jid";
    ...
    ********************************************************************/
        ...
        <img src="./tyyli/kuvat/cal.gif" width="16" height="16" border="0" alt="Klikkaa valitaksesi päivämäärä"></a><br>
