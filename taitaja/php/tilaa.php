<?php
    ...
    if(isset($_GET["id"], $_GET["amount"]))
    {
        ...
        // tiedot databasesta id:llä (rengasmerkki koko tilattu määrä hinta)
        // form jossa asiakkaan tietoja
        // check stock
	    $id = $_GET["id"];
	    $amount = $_GET["amount"];
	    $stock_query = "SELECT `saldo` FROM renkaat WHERE `RengasID`=?";
	    $stock_data = array($id);
	    $stmt = $pdo->prepare($stock_query);
        $stmt->execute($stock_data);
	    $quantity = $stmt->fetch();
        
	    if((int)$quantity < (int)$amount)
	    {
